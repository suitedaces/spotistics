...
const getUserData = async (): Promise<[User, { items: Artist[] }, { items: Track[] }, any, any, { items: RecentlyPlayed[] }]> => {
          ...
          <div className="py-10">
            <h1 className="text-center text-3xl sm:text-4xl lg:text-4xl mr-5">
              ...
              <span className="bg-gradient-to-r from-green-300 via-blue-500 to-purple-600 text-transparent bg-clip-text">
                {data[0].data.display_name.split(' ')[0].concat('!')}
              </span>
              <span>&nbsp;👋</span>
            </h1>
          </div>
...
9                 <Link to={`track/${track.id}`} key={track.id}>
...
0                 <h2 className="hover:text-green-400">See More</h2>
              </Link>
            </div>
          </div>
...
2                 <div className="w-6/12 lg:w-7/12 text-left">SONG</div>
...
3                 <div className="flex justify-between w-full object-contain text-gray-400 pb-4 hover:translate-x-4 transition-transform duration-300 transform" key={song.played_at}>
