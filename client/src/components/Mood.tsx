...
const Mood = () => {
  ...
  const allGenres: string[] = ['Alt-Rock', 'Hip-Hop', 'Indie', 'Pop', 'EDM', 'R&B', 'K-Pop', 'Latin', 'Country', 'Metal', 'Electronic'];
  const [genre, setGenre] = useState<string | null>(null);
  const [mood, setMood] = useState<MoodState | null>(null);
  const [rex, setRex] = useState<Recommendations | null>(null);
  // bugfix
  const transformGenre = (genre: string): string => {
    if (genre === "R&B") {
      return "r-n-b";
    }
    return genre.toLowerCase();
  };
  const setParams = (m: string) => {
    let q: string;
    switch(m) {
      case 'Happy':
        q = 'minValence=0.8';
