...
export const ArtistCard: React.FC<ArtistCardProps> = ({ index, imageURL, itemName }) => {
    return (
...
interface SongCardProps {
    index?: number;
    imageURL: string;
    subItem: string | object[];
    itemName: string;
}
export const SongCard: React.FC<SongCardProps> = ({ index, imageURL, subItem, itemName }) => {
    return (
        <div className='bg-card pb-1.5 mr-3 md:mr-6 mt-4 w-40 md:w-44 shadow-2xl rounded-sm transition-transform duration-200 transform hover:scale-105'>
            <div className='px-4'>
                <div className='image pt-4 relative'>
                    <img src={imageURL} alt="" className='w-40 h-40 object-cover'/>
...
interface TrackItemProps {
    songName: string;
    songArtists: string | object[];
    picURL: string;
}
export const TrackItem: React.FC<TrackItemProps> = ({ songName, songArtists, picURL }) => {
    return (
